...
import { ContratMoeDocument } from '@/lib/pdf/contrat-moe-template'
...
export async function POST(req: NextRequest) {
  ...
  const { error: sendError } = await resend.emails.send({
    ...
    subject: `Contrat de Maîtrise d'Œuvre — ${societe}`,
    html: `
      <div style="font-family: sans-serif; max-width: 520px; margin: 0 auto; color: #2D2D2B;">
        ...
        <div style="background: #f9f8f5; padding: 32px; border: 1px solid #e8e4dd; border-top: none; border-radius: 0 0 8px 8px;">
          <p style="margin: 0 0 16px;">Veuillez trouver en pièce jointe le modèle de contrat de maîtrise d'œuvre de <strong>${societe}</strong>.</p>
          <p style="color: #8A8880; font-size: 13px; margin: 0;">Le document est disponible en pièce jointe au format PDF.</p>
        </div>
      </div>
    `,
    attachments: [{ filename: `Contrat_MOE_${societe.replace(/\s+/g, '_')}.pdf`, content: buffer }],
  })
