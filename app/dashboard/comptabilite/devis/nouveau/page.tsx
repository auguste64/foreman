...
import { createDevisDoc, calcTotauxDoc, formatEurDoc } from '@/lib/supabase/documents'
...
export default function NouveauDevisPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setForm(p => ({ ...p, numero: `DEVIS-${annee}-${(countRes.count || 0) + 1}` }))
  ...
  return (
    <div className="page-enter" style={{ flex: 1, padding: '40px', overflowY: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '55fr 45fr', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <Card title="Informations">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <div>
                ...
                <button type="button" onClick={() => setDatePickerField('date_validite')}
                  style={{ ...inputStyle, textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between' } as React.CSSProperties}
                  onMouseEnter={e => e.currentTarget.style.borderColor = '#ea580c'} onMouseLeave={e => e.currentTarget.style.borderColor = '#1E1E1C'}>
          ...
          <Card title="Notes & conditions">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                ...
                <textarea value={form.notes} onChange={set('notes')} onFocus={focus} onBlur={blur} rows={3} style={{ ...inputStyle, resize: 'vertical' } as React.CSSProperties} placeholder="Informations complémentaires…" />
              </div>
              <div>
                <label style={labelStyle}>Conditions</label>
                <textarea value={form.conditions} onChange={set('conditions')} onFocus={focus} onBlur={blur} rows={2} style={{ ...inputStyle, resize: 'vertical' } as React.CSSProperties} placeholder="Conditions de paiement, délais…" />
              </div>
            </div>
          </Card>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <Link href="/dashboard/comptabilite" style={{ padding: '11px 20px', backgroundColor: 'transparent', color: '#8A8880', border: '1px solid #1E1E1C', borderRadius: 8, fontSize: 14, textDecoration: 'none', fontFamily: 'var(--font-dm-sans), sans-serif' }}>Annuler</Link>
          </div>
        </div>
      ...
      {datePickerField && (
        <DatePickerOverlay
          label={datePickerField === 'date_emission' ? "Date d'émission" : 'Date de validité'}
