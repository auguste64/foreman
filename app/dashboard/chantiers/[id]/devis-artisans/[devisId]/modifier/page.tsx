...
import { updateDevisArtisan } from '@/lib/supabase/devis-artisans'
...
export default function ModifierDevisArtisanPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [artisansData, chantierRes, devisRes] = await Promise.all([
        getArtisans().catch(() => []),
        supabase.from('chantiers').select('nom').eq('id', chantierId).single(),
        supabase.from('devis_artisans').select('*').eq('id', devisId).single(),
      ])
      ...
      if (d) {
        setForm({
          ...
          numero:         d.numero ?? '',
          lot:            d.lot ?? '',
          montant_ht:     d.montant_ht != null ? String(d.montant_ht) : '',
          tva:            d.tva != null ? String(d.tva) : '20',
          montant_ttc:    d.montant_ttc != null ? String(d.montant_ttc) : '',
  ...
  async function handleSave() {
    setSaving(true)
    try {
      await updateDevisArtisan(devisId, {
      ...
      toast.success('Devis artisan mis à jour')
      ...
      <div style={{ backgroundColor: '#111110', border: '1px solid #1E1E1C', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
        ...
        {pdfUrl && pdfName && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            <a
              ...
              style={{ fontSize: '13px', color: '#ea580c', fontFamily: 'var(--font-dm-sans), sans-serif', textDecoration: 'none' }}
        ...
        <label
          style={{
            ...
            padding: '10px 18px',
            backgroundColor: uploading ? '#2a2a27' : '#1E1E1C',
            border: '1px solid #2a2a27',
            borderRadius: '8px',
            fontSize: '13px', fontWeight: 600,
            color: uploading ? '#8A8880' : '#F0EDE6',
            cursor: uploading ? 'not-allowed' : 'pointer',
            fontFamily: 'var(--font-dm-sans), sans-serif',
            transition: 'all 0.15s',
          }}
          onMouseEnter={e => { if (!uploading) { e.currentTarget.style.borderColor = '#ea580c'; e.currentTarget.style.color = '#ea580c' } }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = '#2a2a27'; e.currentTarget.style.color = uploading ? '#8A8880' : '#F0EDE6' }}
        >
          <input
            type="file"
            accept=".pdf"
          ...
          />
          {uploading ? '⏳ Upload…' : pdfUrl ? '📄 Remplacer le PDF' : '📄 Ajouter un PDF'}
        </label>
      </div>
