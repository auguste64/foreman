...
export default function NouvelleFacturePage() {
  ...
  useEffect(() => {
    async function genNumero() {
      ...
      const numero = `FACTURE-${annee}-${(count || 0) + 1}`
      setForm(p => ({ ...p, numero }))
    }
    genNumero()
  }, [])
  ...
  if (loadingDevis) {
    return (
      <div style={{ flex: 1, padding: '40px', fontFamily: 'var(--font-dm-sans), sans-serif', color: '#8A8880', fontSize: '14px' }}>
        Chargement du devis…
      </div>
    )
  }
  ...
  return (
    <div className="page-enter" style={{ flex: 1, padding: '40px', overflowY: 'auto', maxWidth: '800px' }}>
      ...
      {devisId && (
        <p style={{ fontSize: '13px', color: '#ea580c', fontFamily: 'var(--font-dm-sans), sans-serif', margin: '0 0 28px' }}>
          Pré-remplie depuis le devis
        </p>
      )}
      {!devisId && <div style={{ marginBottom: '28px' }} />}
      ...
      <div style={{ backgroundColor: '#111110', border: '1px solid #1E1E1C', borderRadius: '12px', padding: '28px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          <div>
            ...
            <input type="text" value={form.titre} onChange={setField('titre')} style={inputStyle} onFocus={focus} onBlur={blur} placeholder="Acompte 30%" />
          </div>
          <div>
            ...
            <div ref={statutRef} style={{ position: 'relative' }}>
              ...
              {statutOpen && (
                <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, backgroundColor: '#111110', border: '1px solid #1E1E1C', borderRadius: '8px', zIndex: 50, overflow: 'hidden', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                  {['En attente', 'Payée', 'Annulée'].map(s => (
                    <div
                      key={s}
                      onMouseDown={() => { setForm(p => ({ ...p, statut: s })); setStatutOpen(false) }}
                      style={{ padding: '10px 16px', fontSize: '14px', fontFamily: 'var(--font-dm-sans), sans-serif', cursor: 'pointer', color: form.statut === s ? '#ea580c' : '#F0EDE6' }}
          ...
          <div>
            ...
            <input type="date" value={form.date_echeance} onChange={setField('date_echeance')} style={{ ...inputStyle, colorScheme: 'dark' }} onFocus={focus} onBlur={blur} />
          </div>
          <div style={{ gridColumn: '1 / -1' }}>
            ...
            <textarea
              ...
              onFocus={focus}
              onBlur={blur}
              placeholder="Conditions de règlement, coordonnées bancaires..."
            />
          </div>
        </div>
      </div>
